...
function scheduleAutoScan() {
    const interval = parseInt(db.prepare("SELECT value FROM settings WHERE key = 'auto_scan_interval'").get()?.value) || 0;
    ...
    if (interval > 0) {
        ...
        autoScanInterval = setInterval(async () => {
            ...
            const localPath = db.prepare("SELECT value FROM settings WHERE key = 'source_local'").get()?.value;
            if (localPath && fs.existsSync(localPath)) {
                ...
                try {
                    ...
                    console.log(`✅ Auto-scan complete. Found ${library.tracks ? library.tracks.length : 0} tracks.`);
                } catch (err) {
...
app.use('/api/stream', require('./routes/stream'));
    ...
    setInterval(() => {
        const currentInterval = parseInt(db.prepare("SELECT value FROM settings WHERE key = 'auto_scan_interval'").get()?.value) || 0;
        const isScheduled = autoScanInterval !== null;
    
        // If setting changed, reschedule
        if ((currentInterval > 0 && !isScheduled) || (currentInterval === 0 && isScheduled)) {
            scheduleAutoScan();
        }
...
app.use('/api/playcount', authMiddleware);
